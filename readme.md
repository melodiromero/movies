...
+ Limpieza en clave primaria: partiendo del archivo [movies_dataset.csv](/dataset/movies_dataset.csv) se hace uso de microsoft excel para observar detenidamente los datos en las columnas.
Se inicia asi con la columna **id**, *la clave primaria*, tipo entero, filtrándola se obtuvieron registros que poseen valores tipo texto y otros vacios. Fueron descartados, porque además de su clave, esos registros en su columna **title** estaban vacios, almenos, la mayoria, la columna **budget** , *el presupuesto de la pelicula*, tenian valores incorrectos (algunos textos y otros vacios). El campo **genres** en esos registros estanan con datos incorrectos qye no hacian referencia al genero de la pelicula, lo mismo con el campo **original_language**, *el idioma original con el que se grabó* la pelicula. Razones suficientes para descartar asi 46 registros. Estos estan respaldados en ......
...
El ciclo de vida de un proyecto de Machine Learning debe contemplar desde el tratamiento y recolección de los datos (Da y seguidota Engineer stuff) hasta el entrenamiento y mantenimiento del modelo de ML según llegan nuevos datos.
